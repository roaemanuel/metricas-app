    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        <button
          ...
          onMouseLeave={(e) => {
            e.currentTarget.style.background = value > 0 ? color + '22' : 'var(--bg-base)'
...
const EMPTY_FORM = {
  ...
  imagenes_codigos_actualizadas: 0,
  imagenes_peso_optimizado: 0,
  notas: '',
}
...
export default function SistemasIngresarPage() {
  ...
  function addKeyword() {
    ...
    setGa4((g) => ({
      ...g,
      seo_keywords: [...g.seo_keywords, { ...newKw, id: Date.now() }],
    }))
  ...
  function removeKeyword(id) {
    setGa4((g) => ({
      ...g,
      seo_keywords: g.seo_keywords.filter((k) => k.id !== id),
    }))
  }
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div style={{ background: 'var(--bg-surface)', border: `1px solid ${color}33`, borderRadius: 16, padding: '24px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 20 }}>
                    <button
                      onClick={() => setForm((f) => ({ ...f, incidencias_resueltas: Math.max(0, f.incidencias_resueltas - 1) }))}
                      style={{
                        width: 52,
                        height: 52,
                        borderRadius: 14,
                    ...
                    <button
                      onClick={() => setForm((f) => ({ ...f, incidencias_resueltas: f.incidencias_resueltas + 1 }))}
                      style={{
                        width: 52,
                        height: 52,
                        borderRadius: 14,
                      ...
                      onMouseLeave={(e) => {
                        e.currentTarget.style.background = form.incidencias_resueltas > 0 ? color + '22' : 'var(--bg-elevated)'
                        e.currentTarget.style.borderColor = form.incidencias_resueltas > 0 ? color + '66' : 'var(--border)'
                      }}
                    >
                      +
                    </button>
              ...
              <div style={{ background: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: 16, padding: '20px 24px' }}>
                ...
                <textarea
                  ...
                  placeholder="Observaciones técnicas del día…"
                  rows={2}
              ...
              >
                <div>
                  ...
                  <span style={{ fontSize: '0.72rem', color: 'var(--text-muted)', marginLeft: 12, fontFamily: 'var(--font-mono)' }}>
                    · ingreso manual · API automática próximamente
                  </span>
                </div>

                <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                  {existingGa4 && (
                    <span style={{ fontSize: '0.72rem', background: '#10b98120', color: '#10b981', border: '1px solid #10b98144', borderRadius: 99, padding: '2px 10px', fontWeight: 600 }}>
                      ✓ Datos existentes
                    </span>
                  )}
                  <a
              ...
              <div style={{ background: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: 16, overflow: 'hidden' }}>
                ...
                <div style={{ padding: '20px 24px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(190px, 1fr))', gap: 14 }}>
                  {[
                    ...
                    <div key={f.key}>
                      <label style={{ display: 'block', fontSize: '0.73rem', color: 'var(--text-muted)', fontWeight: 600, marginBottom: 6, letterSpacing: '0.05em' }}>
                        {f.label}
                      </label>
                      <input
                        type="number"
                        value={ga4[f.key]}
                        onChange={(e) => setGa4((g) => ({ ...g, [f.key]: e.target.value }))}
              ...
              <div style={{ background: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: 16, overflow: 'hidden' }}>
                ...
                <div style={{ padding: '20px 24px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 14 }}>
                  {[
                    ...
                    <div key={f.key}>
                      <label style={{ display: 'block', fontSize: '0.73rem', fontWeight: 600, marginBottom: 6, color: f.color, letterSpacing: '0.05em' }}>
                        {f.label}
                      </label>
                      <input
                        type="number"
                        ...
                        onChange={(e) => setGa4((g) => ({ ...g, [f.key]: e.target.value }))}
                        placeholder="0"
                        style={{ ...inputSt, borderColor: ga4[f.key] ? f.color + '44' : 'var(--border)' }}
                      />
                    </div>
                  ))}
                </div>
              ...
              <div style={{ background: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: 16, overflow: 'hidden' }}>
                ...
                <div style={{ padding: '20px 24px' }}>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 80px 80px 80px auto', gap: 8, marginBottom: 12 }}>
                    {[
                      { k: 'keyword', pl: 'Keyword' },
                      { k: 'posicion', pl: 'Pos.' },
                      { k: 'clics', pl: 'Clics' },
                      { k: 'impresiones', pl: 'Impr.' },
                    ].map((f) => (
                      <input
                        key={f.k}
                        value={newKw[f.k]}
                        onChange={(e) => setNewKw((k) => ({ ...k, [f.k]: e.target.value }))}
                    ...
                    <button
                      ...
                      style={{
                        padding: '8px 14px',
                        background: newKw.keyword.trim() ? color : 'var(--bg-elevated)',
                        border: 'none',
                        borderRadius: 8,
                        color: newKw.keyword.trim() ? '#000' : 'var(--text-muted)',
                        fontSize: '0.82rem',
                        fontWeight: 700,
                        cursor: 'pointer',
                        whiteSpace: 'nowrap',
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                      {ga4.seo_keywords.map((kw) => (
                        ...
                        >
                          ...
                          <button onClick={() => removeKeyword(kw.id)} style={{ background: 'none', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', fontSize: '0.8rem' }}>
                            ✕
                          </button>
                        </div>
                      ))}
